...
- **規模**：91 位治療師、41 個執業地點、109 個執業關聯（多對多關係；以 `data/data.json` 的 `meta.stats` 為準）
...
| 部署 | 純靜態（GitHub Pages / Vercel），無後端、無 serverless function |
...
│   └── icon.svg            # 應用圖示
├── tests/                  # Vitest 單元測試（hours/geo/data-loader/i18n）
...
npm test             # Vitest 單元測試（hours/geo/data-loader/i18n）
...
### 🔒 個人隱私展示約定
- **單一姓名展示**：在詳情抽屜的地點治療師列表中，**姓名僅展示中文姓名或英文姓名中的一個（優先中文名）**，避免中英文全名並列對照過度曝光。執業牌照號碼（License No.）照常展示以便官方核實。

### 🔐 分析事件隱私約定
- **不上報使用者輸入內容**：Vercel Analytics 事件只記錄「行為發生」（如 `search_used`），**絕不上報搜尋詞等使用者輸入的文字** — 心理健康網站的輸入內容可能含姓名或敏感健康字眼。

### ⏰ 時段篩選與診時解析（hours.js）
- `hours.js` 把 `hours` 自由文字（如「星期一至星期六 12:30-19:30」）解析為結構化時段，支援「現在營業 / 週末開診 / 夜間開診」篩選與列表「營業中」徽章。
...
- **map.js 必須動態載入**：`main.js` 以 `import('./map.js')` 延後載入（maplibre-gl 285KB gzip），列表/搜尋/篩選不等地圖庫；載入前的地圖呼叫經 `mapApi` 包裝為 no-op。勿改回靜態 import。
...
- **AI 對話助理（已移除）**：v2 曾以 `src/copilot.js` + `api/copilot.js`（Vercel 薄代理代管 `DEEPSEEK_API_KEY`）提供免 Key 的 Deepseek Agent，v2.3 已整套移除，搜尋回歸純關鍵字篩選。歷史設計記錄見 `docs/roadmap-v2.md`。除非使用者明確要求，勿重新引入。
- 後端伺服器／資料庫（~90 位治療師、40 地點，靜態 JSON 足夠）。移除薄代理後，本專案回到 100% 純靜態，`data.json` 仍在前端載入。
- 評分/評論、使用者帳號、離線/推播
- 「以治療師為 pin」的地圖視角切換
...
4. 改搜尋 → 桌面 `#desktop-search-input` 與手機 `#mobile-search-input` 兩處都要能觸發 `setQuery`，且 value 互相同步
5. 跑 `npm test` 確認單元測試通過（改 hours/geo/i18n 邏輯時務必補測試）
6. 跑 `npm run build` 確認無語法錯，且 `dist/data/data.json` 存在
7. 跑 `python3 scripts/validate.py` 確認資料一致
8. 改 Vite 設定 → 確認 `closeBundle` hook 仍複製 data/
